...
const DOC_TYPES = [
  { value: 'PHOTO_TOITURE', label: 'Photo toiture', icon: '🏠' },
  { value: 'PHOTO_COMPTEUR', label: 'Photo compteur', icon: '⚡' },
  { value: 'PHOTO_TABLEAU', label: 'Photo tableau électrique', icon: '🔌' },
  { value: 'DEVIS', label: 'Devis signé', icon: '📋' },
  { value: 'MANDAT', label: 'Mandat', icon: '✍️' },
  { value: 'CNI', label: "Pièce d'identité", icon: '🪪' },
  { value: 'FACTURE_EDF', label: 'Facture EDF', icon: '💡' },
...
export default function ClientEspacePage() {
  ...
  const [docForm, setDocForm] = useState({ type: 'PHOTO_TOITURE', nom: '', description: '' })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/documents?commercialId=${id}`).then(r => r.json()),
    ]).then(([p, d]) => {
      setProspect(p)
      setDocs(d)
      setLoading(false)
  ...
  async function saveDocument() {
    ...
    await fetch('/api/documents', {
      ...
      body: JSON.stringify({
        ...
        fichierUrl: preview,
        commercialId: prospect?.commercial?.id,
      }),
    })
    setShowAddDoc(false)
    setPreview(null)
    setDocForm({ type: 'PHOTO_TOITURE', nom: '', description: '' })
    setSaving(false)
    fetch(`/api/documents?commercialId=${id}`).then(r => r.json()).then(setDocs)
  }
  ...
  if (!prospect || prospect.error) return (
  ...
  return (
    <div className="max-w-xl mx-auto p-4 pb-24">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900">{prospect.raisonSociale}</h1>
          <p className="text-xs text-gray-500">Espace documents client</p>
        </div>
      </div>

      {/* Info client */}
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-5">
        <h2 className="text-xs font-semibold text-gray-400 uppercase mb-3">Informations client</h2>
        <div className="space-y-2 text-sm">
          {(prospect.contactPrenom || prospect.contactNom) && (
            <div className="flex gap-2">
              <span className="text-gray-400 w-20 flex-shrink-0">Contact</span>
              <span className="font-medium text-gray-900">{prospect.contactPrenom} {prospect.contactNom}</span>
            </div>
          )}
          {prospect.telephone && (
            <div className="flex gap-2">
              <span className="text-gray-400 w-20 flex-shrink-0">Tél.</span>
              <a href={`tel:${prospect.telephone}`} className="font-medium text-blue-600">{prospect.telephone}</a>
            </div>
          )}
          {prospect.email && (
            <div className="flex gap-2">
              <span className="text-gray-400 w-20 flex-shrink-0">Email</span>
              <a href={`mailto:${prospect.email}`} className="font-medium text-blue-600 truncate">{prospect.email}</a>
            </div>
          )}
          {(prospect.adresse || prospect.ville) && (
            <div className="flex gap-2">
              <span className="text-gray-400 w-20 flex-shrink-0">Adresse</span>
              <span className="text-gray-700">{[prospect.adresse, prospect.codePostal, prospect.ville].filter(Boolean).join(', ')}</span>
            </div>
          )}
        </div>
      </div>
            ...
            return (
              <div key={doc.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                {isImage && doc.fichierUrl && (
                  <img src={doc.fichierUrl} alt={doc.nom} className="w-full h-40 object-cover" />
                )}
                <div className="p-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => { if (fileRef.current) { fileRef.current.accept = '*/*'; fileRef.current.removeAttribute('capture'); fileRef.current.click() } }}
                    className="py-3 border-2 border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-blue-400 hover:text-blue-500 transition-colors"
                  >
                    📁 Choisir un fichier
                  </button>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Photo toiture côté sud"
                />
              </div>
